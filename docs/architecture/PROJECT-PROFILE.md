...
| Branch naming         | `feature/<slug>`, `fix/<slug>`, `chore/<slug>` — one branch per fix, never several fixes on one branch                                                                                                          |
| Ticket prefix         | none                                                                                                                                                                                                            |
| Pull requests         | **not required**, but `main` is gated. Every fix gets its own branch, and nothing reaches `main` without the author's explicit approval — the branch is the unit of review, whether or not it is opened as a PR |
