...
`GoogleService-Info.plist` is committed so the reviewer can build without touching the
Firebase console; the repository is public by the author's decision, and the file contains
no secret — the Firebase API key is a public identifier, and the project's only account is
the shared test number.
