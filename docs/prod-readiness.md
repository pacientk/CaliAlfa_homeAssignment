...
| Remove `+972 52-828-7009` from Firebase Console → Authentication → Sign-in method → Phone → Test phone numbers before any public release                                                     | SPEC-001 §16.4   | KirTer | yes       | [ ]  |
| Restrict the Firebase Web API key in Google Cloud Console → Credentials to the iOS bundle identifier, so the committed key cannot be replayed from anywhere else                             | SPEC-001 §16.4   | KirTer | no        | [ ]  |
