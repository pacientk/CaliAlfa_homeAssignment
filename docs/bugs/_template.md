...
| Originating SPEC    | `SPEC-NNN §X.Y` (the spec section that defines the _intended_ behaviour the code violates) |
...
what the code _should_ do. Quote the SPEC clause if it's terse enough.

**Actual:** one short paragraph stating what the code _currently_ does.
